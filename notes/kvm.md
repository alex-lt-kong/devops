...
```
  * <a id="usb-device-same-vendorId-productId">If USB devices share the same
  vendorId and productId</a>, we need to add an `<address />` section to the
  example, making it something like below:
  ```XML
    <hostdev mode='subsystem' type='usb' managed='yes'>
      <source>
        <vendor id='0x05a3'/>
        <product id='0x9230'/>
        <!-- the extra section, two USB devices should have different
          bus/device combination -->
        <address bus='1' device='3'/>
      </source>
    </hostdev>
  ```

* Whether or not we add an `<address />` section in the `<source />` section,
after we save the XML definition, KVM will generate
one extra `<address />` section under the `<hostdev />` section outside
`<source />`:
```XML
<hostdev mode='subsystem' type='usb' managed='yes'>
  <source>
    <vendor id='0x05a3'/>
    <product id='0x9230'/>
    <!-- necessary only if we want to passthough two USB devices with
    identical vendorId and productId -->
    <address bus='1' device='3'/>
  </source>
  <!-- auto-generated by KVM -->
  <address type='usb' bus='0' port='6'/>
</hostdev>
```
  * Usually we can safely ignore that.

#### Special notes if USB devices have identical vendorId and productId

* The tricky part of having USB devices that have the same vendorId and
productId can be solved by adding an `<address />` section as mentioned
[here](#usb-device-same-vendorId-productId).

* However, the issue will reappear after we re-arrange the order of USB
connectors or add new USB devices. We may notice that some virtual machines
fail to start, complaining that the USB devices being passed through are not
found.

* The solution to this issue, according to multiple experiments, is:
  1. Remove the auto-generated `<address type='usb' />` section. (So that
  KVM can generate it again according to the new order of USB devices)
  1. Change the `bus` and `device` values of the `<accress />` section
  in `<source />`, according to the new output of `lsusb`.
